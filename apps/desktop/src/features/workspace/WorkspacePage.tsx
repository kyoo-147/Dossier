...
export function WorkspacePage() {
  ...
  return (
    <WorkstationShell
      documentRail={
        <DocumentRailShell
          header={<><strong>{fallbackWorkspace.documentTitle}</strong><span>{fallbackWorkspace.subtitle}</span><small>{documentMeta}</small><small>State: {stateLabel}</small></>}
          footer={<><button className="rail-add">＋ <span>Add page</span></button><button className="rail-more">•••</button></>}
        >
      ...
      viewer={
        <DocumentViewerShell
          toolbar={<><div className="toolbar-group"><button>↖</button><button>✋</button></div><div className="toolbar-group toolbar-zoom"><button>−</button><strong>114%</strong><button>＋</button></div><div className="toolbar-spacer" /><button aria-label="Toggle panels">▣</button></>}
          modes={<><button className="viewer-mode viewer-mode--active">▧ Regions</button><button className="viewer-mode">◎ OCR</button><button className="viewer-mode">▦ Table</button><button className="viewer-mode">▧ Heatmap</button><span /><button className="viewer-mode">Fit⌄</button></>}
        >
          <article className="document-paper" aria-label="Document preview">
      ...
      inspector={
        <DocumentInspectorShell
          tabs={<><button className="panel-tab panel-tab--active">Extracted Fields</button><button className="panel-tab">Validation</button><button className="panel-tab">Risk & Anomaly</button><button className="panel-tab">Metadata</button></>}
        >
      ...
      workbench={
        <WorkbenchShell
          mainTabs={<><button className="panel-tab panel-tab--active">Agent Logs</button><button className="panel-tab">Self-Correction</button><button className="panel-tab">Errors & Warnings</button><button className="panel-tab">History</button></>}
          mainContent={
            <div className="log-list">{workspace.logs.map((entry, index) => <div className="log-row" key={`${entry}-${index}`}><time>{`10:21:${String(14 + index * 3).padStart(2, "0")}`}</time><span>{entry}</span></div>)}</div>
          }
          summaryContent={
            <><div className="panel-heading">{workspace.warnings.length} Issues Detected</div>{workspace.warnings.length === 0 ? <p className="empty-copy">No blocking issues.</p> : <ul>{workspace.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul>}</>
          }
          actionPanel={
