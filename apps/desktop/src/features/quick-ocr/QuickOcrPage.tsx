...
export function QuickOcrPage() {
  ...
  return (
    ...
    >
      <section className="quick-dropzone"><span className="quick-drop-icon">OCR</span><div><span className="section-eyebrow">Single document utility</span><h2>Drop a PDF or image here</h2><p>{pickedPath ?? "Printed text and Vietnamese handwriting are supported by the configured provider."}</p></div><button className="button button--primary" onClick={() => void (async () => { const picked = await pickDocumentSource(); if (!picked) return; setPickedPath(picked); await registerDocument({ sourcePath: picked, modeHint: "quick_ocr", pageCount: 1, hasSchema: false }); })()}>Choose a document</button><small>or press Ctrl + O</small></section>
